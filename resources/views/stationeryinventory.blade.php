    ...
    <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
        ...
        <tr>
            ...
            <th>
                <select name="search_stationeryInventoryMaster" id="search_stationeryInventoryMaster" class="form-control form-filter">
                    ...
                    <option value="@php echo $stationeryInventoryMaster->id;@endphp" @php echo $selected;
                        @endphp > @php echo $stationeryInventoryMaster->inventory_desc;@endphp</option>
                    @php }@endphp
                </select>
            </th>
            <th>
                <select name="search_stationeryItem" id="search_stationeryItem" class="form-control form-filter">
                    ...
                    <option value="@php echo $stationeryItem->id;@endphp" @php echo $selected;
                        @endphp > @php echo $stationeryItem->stationery_name;@endphp</option>
                    @php }@endphp
                </select>
            </th>
            <th> <input type="text" name="search_count" id="search_count" value="@php echo $search_count;@endphp" class="form-control form-filter"/></th>
             {{-- For search buttons  --}} 
        ...
        @foreach($allstationeryInventory as $stationeryInventory)
            ...
            <tr class="@php echo $odd_even;@endphp">
                ...
                <td>    @php if ($stationeryInventory->status == 1) {@endphp
                    <span class="label label-sm label-info"> Active </span>
                    @php } else if ($stationeryInventory->status == 2) {@endphp
                    <span class="label label-sm label-warning"> Suspended </span>
                    @php } else if ($stationeryInventory->status == 0) {@endphp
                ...
                <td>
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu pull-left" role="menu">
                            <li>
                                <a data-toggle="modal" href="#view-data-model" data-id="{{ $stationeryInventory->id }}" data-stationeryItem="{{ $stationeryItemNames[$stationeryInventory->item_id] }}" data-stationeryInventoryMasterr="{{ $stationeryInventoryMasterNames[$stationeryInventory->inventory_id] }}" data-count="{{ $stationeryInventory->count }}" class="view-data"><i class="icon-docs"></i> View </a>
                            </li>
                            <li>
                                <a data-toggle="modal" href="#responsive" data-id="{{ $stationeryInventory->id }}" data-stationeryItem="{{  $stationeryInventory->item_id }}" data-stationeryInventoryMasterr="{{ $stationeryInventory->inventory_id }}" data-count="{{ $stationeryInventory->count }}" class="edit-data"><i class="icon-tag"></i> Edit </a>
                            </li>
...
@include('layout.forminputvalue',array('ft'=>'Stationery group Master','fin'=>'view_stationeryInventoryMasterr'))
...
@include('layout.formselect',array('ft'=>'Stationery group Master','fin'=>'edit_stationeryInventoryMaster','data'=>$stationeryInventoryMasters,'index'=>'id','value'=>'inventory_desc','fiv'=>''))
...
@php $js = array("js/stationeryinventory.js");@endphp
